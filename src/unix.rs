...
impl RawPasswordInput for RawModeInput {
    fn new(config: Config) -> io::Result<impl RawPasswordInput> {
        ...
        let input: Box<dyn Read> = match config.clone().input_output.input {
        ...
        let output: Box<dyn Write> = match config.clone().input_output.output {
            InputOutputTarget::Cursor(cursor) => Box::new(cursor),
            InputOutputTarget::FilePath(path) => {
...
mod tests {
    ...
    use crate::{InputOutputTarget, read_password_with_config};
    ...
    fn test_read_password_with_config_errors_with_file_not_found() {
        let config = ConfigBuilder::new()
            .input_output(
                InputOutput::new()
                    .input(InputOutputTarget::FilePath("/does/not/exist".to_string()))
