...
impl RawPasswordInput for RawModeInput {
    fn new(config: Config) -> io::Result<impl RawPasswordInput> {
        let input = match config.input_output.clone().input {
        ...
        let output = match config.input_output.clone().output {
...
#[cfg(test)]
mod tests {
    use crate::{ConfigBuilder, InputOutput, InputOutputTarget, read_password_with_config};
    ...
    fn test_read_password_with_config_errors_with_file_not_found() {
        let config = ConfigBuilder::new()
            .input_output(
                InputOutput::new()
                    .input(InputOutputTarget::FilePath("C:\\not-found.txt".to_string())),
            )
            .build();
