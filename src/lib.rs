...
//! use rpassword::{PasswordFeedback, InputOutput, InputOutputTarget};
//!
//! let mut input = Cursor::new(b"my-password\n".to_vec());
//!
//! let config = rpassword::ConfigBuilder::new()
//!     // Default input/output is the console, but we can pass any file path, cursor or have none
//!     .input_output(
//!         InputOutput::new()
...
pub use config::{Config, ConfigBuilder, InputOutput, InputOutputTarget, PasswordFeedback};
...
/// ```
/// use std::io::{Cursor, Write};
/// use rpassword::{read_password_with_config, ConfigBuilder, InputOutput, InputOutputTarget};
///
/// let mut input = Cursor::new(b"my-password\n".to_vec());
///
...
/// ```
/// use std::io::{Cursor, Write};
/// use rpassword::{ConfigBuilder, InputOutput, prompt_password_with_config, InputOutputTarget};
///
/// let mut input = Cursor::new(b"my-password\n".to_vec());
///
/// let config = ConfigBuilder::new()
...
pub fn prompt_password_with_config(
    ...
    let mut output: Box<dyn Write> = match config.clone().input_output.output {
...
mod tests {
    ...
    fn test_read_password_with_config_with_input_file() {
        ...
        let config = ConfigBuilder::new()
            .input_output(
                InputOutput::new()
                    .input(InputOutputTarget::FilePath(path.clone()))
                    .output(InputOutputTarget::Void),
            )
            .build();
    ...
    fn test_read_password_with_config_with_input_cursor() {
        let cursor = Cursor::new(b"password\n".to_vec());

        let config = ConfigBuilder::new()
            .input_output(
                InputOutput::new()
                    .input(InputOutputTarget::Cursor(cursor))
                    .output(InputOutputTarget::Void),
            )
            .build();
